...
import { TestHelpers, DonationTracker_CampaignCreated } from "generated"
const { MockDb, DonationTracker } = TestHelpers

describe("DonationTracker contract CampaignCreated event tests", () => {
  const mockDb = MockDb.createMockDb()

  const event = DonationTracker.CampaignCreated.createMockEvent({
  ...
  it("DonationTracker_CampaignCreated is created correctly", async () => {
    ...
    let actualDonationTrackerCampaignCreated =
      mockDbUpdated.entities.DonationTracker_CampaignCreated.get(
    ...
    assert.deepEqual(
      actualDonationTrackerCampaignCreated,
      expectedDonationTrackerCampaignCreated,
      "Actual DonationTrackerCampaignCreated should be the same as the expectedDonationTrackerCampaignCreated"
    )
  })
})
